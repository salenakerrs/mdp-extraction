...
def run_pytest(repo_root: str, test_path: str, report_path: str) -> None:
    ...
    retcode = pytest.main(
        [
            ...
            "-p",
            "no:cacheprovider",
            "--cov=/app_mdp/mdp/script/extraction/test",
            "--cov-fail-under=80",
            "--cov-config=/app_mdp/mdp/script/extraction/test/mdp/unit/.coveragerc",
            "--cov-report",
...
def entrypoint(argv: list = sys.argv[1:]):
    ...
    parser.add_argument(
        "--test_path", help="path to run pytest tests", required=False, default="test/mdp/unit"
    )
    parser.add_argument(
        "--report_path",
        help="path to save report file",
        required=False,
        default=f"/Volumes/mdp{ENVIRONMENT}/artifact/test_report",
    )
